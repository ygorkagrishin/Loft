(function () {
    ...
    */
    function handler(e) {
        ...
        return toLookingForTargetSection(value);
    }
    ...
    */
    ...
    function toMoveContainer(coord) {
        container.style.transform = 'translateY(-'+ coord +'px)';  
        container.style.transition = 'all '+ duration +'ms';
    ...
    */
        if (position !== maxPosition && y > newY && SCROLL) {
            toGetCoordTop(sections[++position]); 
        } 
        else if (position !== 0 && y < newY && SCROLL) {
            toGetCoordTop(sections[--position]); 
        }  
    });
